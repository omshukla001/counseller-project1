...
const SRM_BG = 'https://images.unsplash.com/photo-1607237138185-eedd9c632b0b?w=1400&q=80'

export function WhySRM({ onApply }) {
  return (
    <section className="py-14 md:py-20 bg-gradient-to-br from-[#0f172a] via-[#1e3a8a] to-[#0f172a] overflow-hidden relative">
      {/* bg glow */}
      ...
      <div className="max-w-6xl mx-auto px-4 relative">
        <div className="text-center mb-10">
          <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
            className="inline-block bg-yellow-400/20 text-yellow-300 text-xs font-bold px-4 py-1.5 rounded-full border border-yellow-400/30 mb-3">
            ⭐ Featured University
          </motion.span>
          ...
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
            className="text-white/60 mt-2 max-w-lg mx-auto text-xs md:text-sm">
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
            className="relative rounded-3xl overflow-hidden h-64 shadow-2xl">
            <img src={SRM_BG} alt="SRM Campus" className="w-full h-full object-cover" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#0f172a]/90 via-[#0f172a]/40 to-transparent" />
            <div className="absolute bottom-0 left-0 right-0 p-5">
              <p className="text-white font-black text-xl">SRM Institute of Science</p>
              <p className="text-white/70 text-xs flex items-center gap-1 mt-1"><MapPin size={11} /> Kattankulathur, Chennai</p>
              <div className="grid grid-cols-4 gap-2 mt-3">
                {SRM_COLLEGE.stats.map(([v, l]) => (
                  <div key={l} className="bg-white/10 backdrop-blur rounded-lg p-2 text-center border border-white/15">
                    ...
                    <div className="text-white/60 text-[9px]">{l}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {WHY_SRM.map((w, i) => (
              <motion.div key={w.title}
                ...
                <h3 className="font-bold text-white text-xs leading-tight">{w.title}</h3>
                <p className="text-white/55 text-[10px] mt-1">{w.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <p className="text-white/70 text-xs font-semibold mb-3 uppercase tracking-wider">Popular Courses at SRM Chennai</p>
          <div className="flex flex-wrap gap-2">
            {SRM_COLLEGE.courses.map(c => (
              <span key={c} className="flex items-center gap-1.5 text-xs text-white bg-white/10 border border-white/15 px-3 py-1.5 rounded-full">
    ...
    <section className="py-14 md:py-20 bg-[#f0f4ff] overflow-hidden">
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-8 md:mb-12">
          <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
            className="inline-block bg-blue-100 text-[#1e3a8a] text-xs font-bold px-4 py-1.5 rounded-full border border-blue-200 mb-3">
            Why Bangalore?
          </motion.span>
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            className="text-2xl md:text-4xl font-black text-gray-900">India's Engineering Capital</motion.h2>
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
            className="text-gray-500 mt-2 max-w-lg mx-auto text-xs md:text-sm px-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5">
          {WHY_BLR.map((w, i) => (
            <motion.div key={w.title}
              ...
              <h3 className="font-bold text-gray-900 text-xs md:text-sm leading-tight">{w.title}</h3>
              <p className="text-gray-500 text-[10px] md:text-xs mt-1 leading-snug">{w.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-[#0f172a]/85 via-[#1e3a8a]/50 to-transparent flex items-center px-10">
            <div>
              <p className="text-white font-black text-3xl">30,000+ Students</p>
              <p className="text-blue-400 font-semibold mt-1">Successfully Placed in Top Colleges</p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {EXPERTISE.map((e, i) => (
            <motion.div key={e.title}
              ...
              <p className="text-gray-500 text-xs mt-1.5 leading-relaxed">{e.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
        ...
        <div className="relative overflow-hidden">
          <AnimatePresence mode="wait">
            <motion.div key={idx}
              ...
              <p className="text-gray-600 text-base md:text-lg italic mb-8 max-w-2xl mx-auto leading-relaxed">"{t.text}"</p>
              <div className="flex items-center justify-center gap-4">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[#1e3a8a] text-sm font-medium">{t.college}</p>
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
